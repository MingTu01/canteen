/**
 * 餐别配色与图标配置(对齐 H5 useMealConfig.ts)
 *
 * 颜色规范:早餐橙、午餐绿、晚餐紫
 * 用于胶囊 + 图标 + badge + 日期栏圆点
 */
...
/** 获取餐别图标的颜色 */
export const mealIconColor = (mealType: number): string => getMealColor(mealType).text

/** 获取餐别圆点颜色(日期竖列用,与菜单页胶囊一致) */
export const mealDotColor = (mealType: number): string => getMealColor(mealType).text
