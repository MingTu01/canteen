 ...
 *
 * 模板字段约定(在公众号后台配置模板时按此字段命名):
 * 通知模板:{{title.DATA}} {{content.DATA}} {{time.DATA}}
 * 订单模板:{{orderDate.DATA}} {{mealType.DATA}} {{amount.DATA}} {{time.DATA}}
 */
@Service
public class WechatNotifyService {
     ...
     */
    @SuppressWarnings("unchecked")
    private void sendTemplateMessage(String openid, String templateId, Map<String, Object> data, String url) {
        ...
        try {
            ...
            if (result == null) {
                log.warn("微信模板消息发送返回空响应,openid={}", openid);
                return;
            }
            Object errcode = result.get("errcode");
            if (errcode != null && !"0".equals(errcode.toString())) {
                // 常见错误:43004(用户未关注)、40037(模板ID无效)、40003(openid 无效)
                log.warn("微信模板消息发送失败: openid={}, errcode={}, errmsg={}",
                        openid, errcode, result.get("errmsg"));
            }
        } catch (Exception e) {
            log.warn("微信模板消息发送异常: openid={}, error={}", openid, e.getMessage());
        }
    }
    ...
    /**
     * 通知/公告/活动发布时,向门店所有已绑定微信的员工推送模板消息。
    ...
    public void notifyNotificationPublished(Notification notification) {
        ...
        log.info("推送微信通知: storeId={}, type={}, 标题={}, 推送人数={}",
                notification.getStoreId(), typeLabel, title, boundEmployees.size());

        for (Employee emp : boundEmployees) {
            try {
                sendTemplateMessage(emp.getWxOpenid(), templateNotifyId, data, url);
            } catch (Exception e) {
                log.warn("推送微信通知异常: employeeId={}, error={}", emp.getId(), e.getMessage());
            }
        }
    }
    ...
    /**
     * 订单创建成功后,向员工推送微信通知(含订单日期、餐次、金额、取餐码)。
    ...
    public void notifyOrderCreated(Order order, Employee employee) {
        ...
        log.info("推送微信订单通知: employeeId={}, orderId={}, 日期={}, 餐次={}",
                employee.getId(), order.getId(), dateStr, mealTypeStr);

        try {
            sendTemplateMessage(employee.getWxOpenid(), templateOrderId, data, url);
        } catch (Exception e) {
            log.warn("推送微信订单通知异常: employeeId={}, error={}", employee.getId(), e.getMessage());
        }
    }
}
